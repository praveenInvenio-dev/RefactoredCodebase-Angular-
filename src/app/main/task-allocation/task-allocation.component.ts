...
import { taskAllocConstants } from "./../../constants/taskAllocation.constants";
...
export class TaskAllocationComponent implements OnInit {
  ...
  constructor(private activatedRoute: ActivatedRoute, public UserManagementService:UserManagementService,  private router: Router, public notifierService:NotifierService) {
  ...
  getTaxType(index) {
    ...
    this.userAuthIndex = this.newEditUser.findIndex((user)=> user["AuthUsrno"] === this.queryParam && user['Taxtp'] === this.taxTypesList[this.taxTraker]['Taxtp'])
  ...
  getEditUserByTaxType2(taxType) {
    this.UserManagementService.getDetailsByBranch(taxType).subscribe(res => 
      {
        console.log(res)
        this.editGetVar = res
  ...
  getTaskAllocUSerInfo() {  
    ...
    this.UserManagementService.getTaskAllocAllUsers(localStorage['fn'],localStorage['eu']).subscribe(res =>{
      ...
      if(  this.userAuthIndex < 0) {
        ...
        this.getEditUserByTaxType2(this.taxTypesList[this.taxTraker]["Taxtp"])    
      }
      else{
        this.authSeqNo = this.newEditUser[this.userAuthIndex]["AuthSeqNo"]
        // debugger
  ...
  authorizeReturns(p,type) {
      ...
      if(type === "return") {
        ...
        let filtered_array = this.fiscalYearDetails.filter((authorizedReturn)=>  {
          return authorizedReturn["ReturnCb"] === "X" || authorizedReturn["ObjectionCb"] === "X"
        })
        ...
        this.UserManagementService.authorizeData(rebuildServiceArray, {
          "Taxtp":this.taxTypesList[this.taxTraker]['Taxtp'],"BranchNo":this.pageBranchName ['BranchNo'],"BranchNm":this.pageBranchName ['BranchNm'],"AuthusrTin": this.queryParam,"AuthusrName": this.queryParamName,"AuthSeqNo": this.authSeqNo, 
        }).subscribe(res => {
          if(this.notifictionStatus == false) {
            this.notifierService.notify("success",localStorage['lang'] == 'ar' ? taskAllocConstants['taskSuccessfullyDelinked']['arb'] : taskAllocConstants['taskSuccessfullyDelinked']['eng'] )
          }
          else if (this.notifictionStatus == true){
            this.notifierService.notify("success",localStorage['lang'] == 'ar' ? taskAllocConstants['taskSuccessAssigned']['arb'] : taskAllocConstants['taskSuccessAssigned']['eng'] )
          }
  ...
  getEditUserByTaxType(taxTypeIndex) {
    ...
    this.UserManagementService.getEditUserApi(this.newEditUser[this.userAuthIndex],this.taxTypesList[taxTypeIndex]).subscribe(res=>
      {
        console.log(res)
        this.editGetVar = res
