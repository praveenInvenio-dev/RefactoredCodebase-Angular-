...
import { userManagementConstants } from 'src/app/constants/userManagement.constants';
...
export class AddUserComponent implements OnInit {
  ...
  invalidEmail = localStorage['lang'] == 'ar' ? assUserConstants.invalidEmail['arb'] : assUserConstants.invalidEmail['eng'] 
  pleaseEnterEmail  = localStorage['lang'] == 'ar' ? assUserConstants.pleaseEnterEmail['arb'] : assUserConstants.pleaseEnterEmail['eng'] 
  ...
  ngOnInit(): void {
    ...
    console.log(moment(new Date(greg_date.gy,greg_date.gm-1,greg_date.gd)).valueOf())
  ...
  submit(){
    ...
    if(this.form.valid && this.isIDNumberValid){
      ...
      console.log(data)
      this.UserManagementService.postUserDetails(data,this.uid).subscribe((res)=>{
...
isValidCheck()
{
  if(this.form.value.idType === "ZTPAU3" && (this.form.value.idNo.length > 6 && this.form.value.idNo.length <15))
  {
  ...
  navigateToUsermanagement() {
    ...
    ValidateUserIDandDob() {
      ...
      console.log(((this.form.value.idType === "ZTPAU1" || this.form.value.idType === "ZTPAU2") && this.form.value.idNo?.length !== 10) || (this.form.value.idType === "ZTPAU3" && (this.form.value.idNo.length < 7 || this.form.value.idNo.length >14)))
      if(((this.form.value.idType === "ZTPAU1" || this.form.value.idType === "ZTPAU2") && this.form.value.idNo?.length !== 10) || (this.form.value.idType === "ZTPAU3" && (this.form.value.idNo.length < 7 || this.form.value.idNo.length >14))){
        return false;
      }else if(this.form.value.idType === "ZTPAU1" && this.form.value.idNo.charAt(0) !== '1'){
        return false;
      }else if(this.form.value.idType === "ZTPAU2" && this.form.value.idNo.charAt(0) !== '2'){
        return false;
      }
      if(this.form.value['dob']['calendarName']==="Islamic") {
        const gre_date = toGregorian(this.form.value['dob']['calendarStart']['year'],this.form.value['dob']['calendarStart']['month'],this.form.value['dob']['calendarStart']['day'])
        dob = moment(new Date(gre_date['gy'],gre_date['gm']-1,gre_date['gd'])).format("YYYYMMDD")
      }
      else {
       dob = moment(new Date(this.form.value['dob']['calendarStart']['year'],this.form.value['dob']['calendarStart']['month']-1,this.form.value['dob']['calendarStart']['day'])).format("YYYYMMDD")
       
      } 
      this.UserManagementService.ValidateTinAndDob(this.form.value.idType,this.form.value.idNo,dob).subscribe(res=> 
