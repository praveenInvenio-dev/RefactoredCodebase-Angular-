...
import { userManagementConstants } from "./../../constants/userDetails.constants";
...
export class UserdetailsComponent implements OnInit {
  ...
  constructor(private router: Router, private activatedRoute: ActivatedRoute, public UserManagementService: UserManagementService, public notifierService: NotifierService, public appSrv: AppService) {
  ...
  ngOnInit(): void {
    ...
    this.activateButtonVar = localStorage['lang'] == 'ar' ? userManagementConstants['Activate']['arb']: userManagementConstants['Activate']['eng']
    this.deactivateBUttonVar = localStorage['lang'] == 'ar' ? userManagementConstants['Deactivate']['arb']: userManagementConstants['Deactivate']['eng']
  }

  gotoTaskAlloc(authUsri, name) {

    this.router.navigate(['/mains/taskallocation'], { queryParams: { page: authUsri, name: this.queryParamName } })
  }

  gotoEditUser() {
    this.router.navigate(['/mains/edituser'],{ queryParams: { page: this.queryParam, name: this.queryParamName} })
  ...
  getUserDetails() {
    this.UserManagementService.getInitialData(localStorage['gpart']).subscribe(res => {
      console.log(res)
      ...
      let indexFound = userSet.findIndex((user) => user["AuthUsrno"] === this.queryParam)
      ...
      if (indexFound !== -1) {
        ...
        this.UserManagementService.getTaskAllocAllUsers(localStorage['fn'], localStorage['eu']).subscribe(res => {
          ...
          this.linkedTaxPayerArray = newEditUser.filter(user => user["AuthUsrno"] === this.queryParam)
        }, err => {
          console.log(err)
        })
      }
        ...
        this.UserManagementService.getDeletedUsers().subscribe(res => {
          ...
          let deactivatedIndexFound = inactiveUserSet.findIndex((user) => user["AuthUsrno"] === this.queryParam)
  ...
  modalConfirm() {
    ...
    if (!this.showTaskAlloc) {
      this.activateUser(this.color2['AuthUsrno'], this.color2['IdType'], this.color2['IdNumber'], this.color2['Mobileno'], this.color2['Name'])
    } else {
      this.deleteUser(this.color['AuthUsrno'],
        this.color['IdNumber'],
        this.color['Emailid'],
        this.color['UserNo'],
        this.color['BranchNo'])
  ...
  activateUser(i, j, k, l, m) {
    ...
    this.UserManagementService.addDeletedUser(i, j, k, l, m).subscribe(res => {
      console.log("the res for activate is" + res)



      this.closeModal('sukukBondsCard')
      this.notifierService.notify("success", localStorage['lang'] == 'ar' ? userManagementConstants['userSuccessfullyActivated']['ar'] : userManagementConstants['userSuccessfullyActivated']['eng'])
      this.getUserDetails()
    }, err => {
      console.log(err)
  ...
  deleteUser(authId, IdNumber, Emailid, UserNo, BranchNo) {
    ...
    this.UserManagementService.deActivateUser(authId, IdNumber, Emailid, UserNo, BranchNo).subscribe(res => {
      ...
      this.notifierService.notify("success", localStorage['lang'] == 'ar' ? this.userManagementConstants['userSuccessfullyDeleted']['arb'] : this.userManagementConstants['userSuccessfullyDeleted']['eng'])
      this.getUserDetails();
    }, err => {
      console.log(err)
      this.notifierService.notify("error", err["error"]["error"]["innererror"]["errordetails"][0]["message"])
