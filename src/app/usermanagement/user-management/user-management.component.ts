...
export class UserManagementComponent implements OnInit {
  ...
  openUserDetails(authUsr, name, branch) {
    this.router.navigate(["/mains/userdetails"], {
      queryParams: { page: authUsr, name: name, branch: branch },
    });
  }
  ...
  activateUser(authUsrNo, idType, idNumber, mobileNo, name) {
    this.UserManagementService.addDeletedUser(
      authUsrNo,
      idType,
      idNumber,
      mobileNo,
      ...
      (res) => {
        ...
        this.notifierService.notify(
          "success",
          this.lang["userSuccessfullyActivated"]
        );
      },
      (err) => {
        this.notifierService.notify(
      ...
      (res) => {
        ...
        this.notifierService.notify(
          ...
          localStorage["lang"] == "ar"
            ? this.umConstants["arb"]["userSuccessfullyDeleted"]
            : this.umConstants["eng"]["userSuccessfullyDeleted"]
        );
      },
      (err) => {
        this.notifierService.notify(
  ...
  goToTaskAlloc(event, authUsri, name) {
    ...
    this.router.navigate(["/mains/taskallocation"], {
      queryParams: { page: authUsri, name: name },
    });
    this.selectedUserIndex = null;
  }
  ...
  getUserInfo(userName) {
    this.UserManagementService.getInitialData(userName).subscribe(
      (res) => {
        this.getDelUsers();
        if (res != null && res["d"] != null) {
          if (
            res["d"]["UserSet"] != null &&
            res["d"]["UserSet"]["results"] != null
          ) {
            this.getUserResponse = res["d"]["UserSet"]["results"];
          }
          if (
            res["d"]["UserBranchSet"] != null &&
            res["d"]["UserBranchSet"]["results"] != null
          ) {
            this.branchList = res["d"]["UserBranchSet"]["results"];
          }
          if (
            res["d"]["IdSet"] != null &&
            res["d"]["IdSet"]["results"] != null
          ) {
  ...
  //Get fbgUUID api1
  getFgbuid() {
    this.UserManagementService.getFbgUid().subscribe(
      (res) => {
        if (res != null && res["d"] != null) {
          this.getFgbuid2(
            res["d"]["Val1"],
  ...
  getFgbuid2(euser1, euser2, euser3, euser4, euser5) {
    this.UserManagementService.getFbgUid2(
      euser1,
      euser2,
      euser3,
      euser4,
    ...
    this.UserManagementService.getDeletedUsers().subscribe(
      (res) => {
        ...
        for (let i = 0; i < this.getUserResponseInactive.length; i++) {
          ...
          let inactiveBranchIndex = this.branchList.findIndex(
            (index) => index["BranchNo"] == temp_branch_id
          );
          this.getUserResponseInactive[i]["Branch"] = this.branchList[
            inactiveBranchIndex
          ]["BranchNm"];
        }

        // Combine both lists
        this.getUserResponseAll = [
        ...
        for (let i = 0; i < this.getUserResponseDisplayVar.length; i++) {
          let authUserNofromDisplayVar = this.getUserResponseDisplayVar[i][
            "AuthUsrno"
          ];
          this.indexFound = this.taskallocUsersArray.findIndex(
            (index) => index["AuthUsrno"] === authUserNofromDisplayVar
          );
          let uniqueChars: any = [];
          if (this.indexFound >= 0) {
            this.filt = this.taskallocUsersArray.filter(
              (record) => record["AuthUsrno"] == authUserNofromDisplayVar
            );
            ...
            for (let k = 0; k < this.filt.length; k++) {
              this.list =
                this.list +
                this.filt[k]["LinkedActivity"] +
                (k !== this.filt.length - 1 ? ", " : "");
            }
  ...
  getModal(event, cardType) {
    event.stopPropagation();
    const status = this.getUserResponseDisplayVar[this.selectedUserIndex][
      "status"
    ];
    if (!status) {
      this.modalConfirm();
